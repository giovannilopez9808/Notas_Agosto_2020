...
def plot(ax,energy,color,ylabel,yticks,x):
    x=x*1e-5
    ax.set_xlim(0,x[-1])
    ax.set_ylabel(ylabel)
    ax.plot(x,energy,color=color,lw=3)
    if ax!=ax3:
        ax.set_xticks([])
...
plot(ax1,e_kin,"#8d0801","Kinetic energy",np.arange(0.9,1.02,0.02),walks)
plot(ax2,e_pot,"#001427","Potencial energy",np.arange(-1,-0.87,0.02),walks)
plot(ax3,e_tol,"#1b4332","Total energy",np.arange(-0.05,0.07,0.02),walks)
plt.xlabel("Walks (10$^{5}$)")
