...
print("Clasico",round(np.mean(clasic),3),round(np.std(clasic),4))
print("Quantum",round(np.mean(quantum),3),round(np.std(quantum),4))
print("Quantum IBM",round(np.mean(quantum_ibm),3),round(np.std(quantum_ibm),4))
x=np.arange(np.size(clasic))
plt.xlim(0,1000)
plt.scatter(x,clasic,marker=".",color="#03071e",label="Clásico CCGN")
